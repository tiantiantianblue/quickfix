...
namespace FIX
{
		...
		throw(FIX::Exception)
	{
		...
		if (pSessionDD  && pSessionDD->m_hasVersion)
		{
		...
		int field = 0;
		if ((pSessionDD && pSessionDD->m_checkFieldsOutOfOrder) ||
			(pAppDD  && pAppDD->m_checkFieldsOutOfOrder))
		{
			if (!message.hasValidStructure(field))
				throw TagOutOfOrder(field);
		}

		if (pAppDD  && pAppDD->m_hasVersion)
		{
	...
	void DataDictionary::iterate(const FieldMap& map, const MsgType& msgType) const
	{
		...
		for (FieldMap::iterator i = map.begin(); i != map.end(); ++i)
		{
			...
			if (!m_beginString.getValue().empty() && shouldCheckTag(field))
			{
	...
	void DataDictionary::readFromURL(const std::string& url)
		throw(ConfigError)
	{
		auto pDoc = std::make_shared<PUGIXML_DOMDocument>();
