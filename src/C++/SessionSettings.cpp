...
namespace FIX
{
		...
		throw(ConfigError)
	{
		...
		for (const auto sec: section)
		{
			...
			SessionID sessionID(dict.getString(BEGINSTRING), dict.getString(SENDERCOMPID),
				dict.getString(TARGETCOMPID), sessionQualifier);
	...
	void SessionSettings::setDeault(const Dictionary& defaults) throw(ConfigError)
	{
		m_defaults = defaults;
	}
